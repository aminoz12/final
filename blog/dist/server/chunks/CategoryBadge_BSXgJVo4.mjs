import { c as createAstro, a as createComponent, m as maybeRenderHead, b as addAttribute, r as renderTemplate } from './astro/server_BNe_LUEH.mjs';
import 'kleur/colors';
import 'clsx';
...
const $$CategoryBadge = createComponent(($$result, $$props, $$slots) => {
  ...
  const { name, color, size = "md", className = "" } = Astro2.props;
  function getColorClasses(hexColor, size2) {
    ...
    return colorMap[hexColor.toUpperCase()]?.[size2] || defaultClasses[size2];
  }
  const sizeClasses = {
  ...
  return renderTemplate`${maybeRenderHead()}<span${addAttribute(`inline-flex items-center rounded-full font-medium border backdrop-blur-sm transition-all duration-300 hover:scale-105 ${sizeClasses[size]} ${colorClasses} ${className}`, "class")}> ${name} </span>`;
}, "C:/Users/pc/Desktop/PROJECT/ASHRAF/blog/src/components/CategoryBadge.astro", void 0);

export { $$CategoryBadge as $ };
