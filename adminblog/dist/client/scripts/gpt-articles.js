...
// Wait for GPT5Service to be available
let GPT5ServiceAvailable = false;
let checkInterval;

function waitForGPT5Service() {
  if (typeof GPT5Service !== 'undefined') {
    console.log('✅ GPT5Service is now available');
    GPT5ServiceAvailable = true;
...
// Check every 100ms for GPT5Service availability
checkInterval = setInterval(waitForGPT5Service, 100);

// Set a timeout to prevent infinite waiting
setTimeout(() => {
  if (!GPT5ServiceAvailable) {
    console.error('❌ GPT5Service not available after 5 seconds');
    clearInterval(checkInterval);
  }
}, 5000);
...
class GPT5ArticlesManager {
  constructor() {
    ...
    try {
      ...
      if (typeof GPT5Service === 'undefined') {
        throw new Error('GPT5Service is not available. Make sure gpt5-service.js is loaded before gpt-articles.js');
      }
      
      console.log('GPT5ArticlesManager: GPT5Service class found, creating instance...');
      this.gptService = new GPT5Service();
      console.log('GPT5ArticlesManager: GPT5Service instance created:', this.gptService);
      
      // Verify the instance has the required methods
      if (typeof this.gptService.cleanupBlobUrl !== 'function') {
        console.warn('⚠️ GPT5Service instance missing cleanupBlobUrl method');
        console.log('🔍 Available methods:', Object.getOwnPropertyNames(this.gptService));
        console.log('🔍 Prototype methods:', Object.getOwnPropertyNames(Object.getPrototypeOf(this.gptService)));
        
        // Try to recreate the instance if it's missing methods
        console.log('🔄 Attempting to recreate GPT5Service instance...');
        try {
          this.gptService = new GPT5Service();
          if (typeof this.gptService.cleanupBlobUrl === 'function') {
            console.log('✅ GPT5Service instance recreated successfully with cleanupBlobUrl method');
          } else {
            console.error('❌ GPT5Service instance still missing cleanupBlobUrl method after recreation');
          }
        } catch (recreateError) {
          console.error('❌ Failed to recreate GPT5Service instance:', recreateError);
        }
      } else {
        console.log('✅ GPT5Service instance has cleanupBlobUrl method');
      }
  ...
  // Set up global event listeners for categories updates
  setupGlobalEventListeners() {
    // Listen for localStorage changes to refresh categories
    window.addEventListener('storage', (e) => {
      if (e.key === 'userCategories' && this.refreshCategories) {
  ...
  cleanupBlobUrls() {
    console.log('🧹 cleanupBlobUrls called');
    console.log('🔍 this.gptService:', this.gptService);
    console.log('🔍 typeof this.gptService:', typeof this.gptService);
    
    if (this.currentArticle && this.currentArticle.generatedImage && this.currentArticle.generatedImage.blobUrl) {
      console.log('🔍 Found blob URL:', this.currentArticle.generatedImage.blobUrl);
      
      // Try multiple ways to cleanup the blob URL
      let cleanupSuccessful = false;
      
      // Method 1: Use the instance method if available
      if (this.gptService && typeof this.gptService.cleanupBlobUrl === 'function') {
        console.log('✅ Calling cleanupBlobUrl on gptService instance');
        try {
          this.gptService.cleanupBlobUrl(this.currentArticle.generatedImage.blobUrl);
          cleanupSuccessful = true;
          console.log('✅ Cleanup successful with instance method');
        } catch (error) {
          console.warn('⚠️ Error calling gptService.cleanupBlobUrl:', error);
        }
      }
      
      // Method 2: Try creating a new instance if the current one is not available
      if (!cleanupSuccessful && typeof GPT5Service !== 'undefined') {
        console.log('✅ Creating new GPT5Service instance for cleanup');
        try {
          const tempService = new GPT5Service();
          if (typeof tempService.cleanupBlobUrl === 'function') {
            tempService.cleanupBlobUrl(this.currentArticle.generatedImage.blobUrl);
            cleanupSuccessful = true;
            console.log('✅ Cleanup successful with new instance');
          } else {
            console.warn('⚠️ New instance also missing cleanupBlobUrl method');
          }
        } catch (error) {
          console.warn('⚠️ Error creating new GPT5Service instance:', error);
        }
      }
      
      // Method 3: Direct URL.revokeObjectURL as fallback
      if (!cleanupSuccessful) {
        console.log('🔄 Using direct URL.revokeObjectURL as fallback');
        try {
          if (this.currentArticle.generatedImage.blobUrl.startsWith('blob:')) {
            URL.revokeObjectURL(this.currentArticle.generatedImage.blobUrl);
            console.log('✅ Blob URL cleaned up directly');
            cleanupSuccessful = true;
          }
        } catch (error) {
          console.warn('⚠️ Error using direct URL.revokeObjectURL:', error);
        }
      }
      
      if (!cleanupSuccessful) {
        console.error('❌ All cleanup methods failed');
        console.log('🔍 gptService methods:', this.gptService ? Object.getOwnPropertyNames(this.gptService) : 'undefined');
        console.log('🔍 gptService prototype methods:', this.gptService ? Object.getOwnPropertyNames(Object.getPrototypeOf(this.gptService)) : 'undefined');
        console.log('🔍 GPT5Service available:', typeof GPT5Service !== 'undefined');
        if (typeof GPT5Service !== 'undefined') {
          console.log('🔍 GPT5Service prototype methods:', Object.getOwnPropertyNames(GPT5Service.prototype));
        }
      } else {
        console.log('✅ Blob URL cleanup completed successfully');
      }
    } else {
      console.log('🔍 No blob URL to cleanup');
    }
  }
  ...
  async publishArticle() {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Error during blob cleanup, continuing with publication:', cleanupError);
        // Continue with publication even if cleanup fails
      }
...
// Note: Initialization is now handled at the top of the file to ensure proper loading order
// The old DOMContentLoaded listener has been replaced with a more robust loading mechanism