...
class RealTimeChat {
  constructor() {
    ...
    // Initialize GPT5Service instance
    if (typeof GPT5Service !== 'undefined') {
      this.gptService = new GPT5Service();
      console.log('✅ GPT5Service initialized in RealTimeChat');
    } else {
      console.warn('⚠️ GPT5Service not available');
    }
  ...
  startConversationStream(conversationId) {
    ...
    // Start new stream
    const url = `/api/chat-stream?conversation_id=${conversationId}`;
  ...
  async generateAIResponse() {
    ...
    try {
      ...
      // Call GPT-5 service for chat response
      if (this.gptService && typeof this.gptService.generateArticle === 'function') {
        try {
          const aiResponse = await this.gptService.generateArticle(
            `Contexte de la conversation:\n${context}\n\nRéponds de manière naturelle et utile comme Sarah, l'assistante de support. Réponse courte et directe.`,
            { length: 'short', tone: 'friendly' }
          );
          
          if (aiResponse && aiResponse.success && aiResponse.data && aiResponse.data.content) {
            // Extract the content from the response
            const responseContent = aiResponse.data.content;
            ...
            console.warn('⚠️ AI response not in expected format:', aiResponse);
          }
        } catch (error) {
