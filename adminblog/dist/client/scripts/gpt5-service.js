...
class GPT5Service {
  ...
  getApiKey() {
    ...
    console.log('GPT Service: Using fallback API key');
    return fallbackKey;
  }
    ...
    try {
      ...
      if (data.data && data.data[0] && data.data[0].b64_json) {
        ...
        try {
          ...
          // Final safety check: ensure we never return blob URLs
          if (imageData.url && imageData.url.startsWith('blob:')) {
            console.warn('⚠️ Safety check: Converting blob URL to data URL for permanence');
            const dataUrl = `data:image/png;base64,${base64Data}`;
            imageData.url = dataUrl;
            imageData.isDataUrl = true;
            imageData.isPermanent = true;
            console.log('✅ Converted blob URL to data URL for permanence');
          }
          ...
          return {
            ...
            data: {
              ...
              size: Math.ceil(base64Data.length * 0.75), // Approximate size
              type: 'image/png',
              prompt: imagePrompt,
              style: style,
              isDataUrl: true,
              isPermanent: true, // Data URLs are permanent
              isOpenAIUrl: false,
              isLocal: false,
              alt: `Image générée par IA pour: ${prompt}`,
              caption: `Image générée automatiquement par DALL-E 3`,
              fallbackType: 'data-url-fallback'
            }
          };
        }
      } else {
  ...
  async saveBase64ImageAsFile(base64Data, prompt) {
    try {
      ...
      // Validate that we're not returning a blob URL
      if (imageData.url && imageData.url.startsWith('blob:')) {
        console.warn('⚠️ Warning: Server returned blob URL, converting to data URL for permanence');
        const dataUrl = await this.blobToDataUrl(imageBlob);
        imageData.url = dataUrl;
        imageData.isDataUrl = true;
        console.log('✅ Converted blob URL to data URL for permanence');
        ...
        console.warn('⚠️ Server upload failed, using data URL as fallback:', uploadError);
        ...
        // Fallback: create a data URL instead of blob URL for permanence
        console.log('🔄 Creating fallback data URL...');
        const dataUrl = await this.blobToDataUrl(imageBlob);
        
        console.log('🔄 Created fallback data URL, length:', dataUrl.length);
      ...
      // Note: Fallback to data URL is already handled above in the catch block
