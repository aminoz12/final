...
async function loadUserCategories() {
  try {
    ...
    // Try to load from database API first (development only)
    try {
      const response = await fetch('/api/categories');
      if (response.ok) {
        const result = await response.json();
        if (result.success && result.categories) {
          console.log('✅ Catégories chargées depuis la base de données:', result.categories.length);
          
          // Update localStorage with fresh data
          localStorage.setItem('userCategories', JSON.stringify(result.categories));
          
          // Update category select
          updateCategorySelect(result.categories);
          return;
        } else if (result.error && result.error.includes('production')) {
          console.log('ℹ️ Mode production détecté, utilisation du localStorage');
        }
      }
    } catch (apiError) {
      console.warn('⚠️ Échec du chargement depuis l\'API, utilisation du localStorage:', apiError);
    }
